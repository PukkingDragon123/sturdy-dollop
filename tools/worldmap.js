...
const w = load(['js/px/pal.js', 'js/px/px.js', 'js/world/zones.js', 'js/world/tiles.js',
                'js/world/world.js', 'js/world/light.js', 'js/world/village.js', 'js/world/gen.js']);
const KD = w.KD, Wd = KD.World, Gen = KD.Gen, T = KD.Tiles;

const t0 = Date.now();
const n = Gen.begin(KD.Zones.WORLD_W, KD.Zones.WORLD_H, seed);
let s;
...
console.log('spawn ' + JSON.stringify(Gen.meta.spawn) + '  village houses ' + (Gen.meta.village ? Gen.meta.village.buildings.length : 0) +
...
const openCol = (x0, x1, y0, y1) => {
  let open = 0, all = 0;
  for (let y = y0; y < y1; y++) for (let x = x0; x < x1; x++) { all++; if (!T.isSolid(Wd.at(x, y))) open++; }
  return (open / all * 100).toFixed(1) + '%';
};
console.log('\nzones (open space in the top 120 rows):');
for (const z of KD.Zones.Z) {
  console.log('  ' + z.id.padEnd(8) + ' x' + z.x0 + '-' + z.x1 + '  ' + openCol(z.x0, z.x1, 30, 150).padStart(6) + '   ' + z.name);
}
console.log('\ntile mix (top 14):');
...
console.log(write('world-deep.png', Math.max(0, Wd.W - 220), 360, 220, 96, 4, true));
console.log(write('world-village.png', Math.max(0, KD.Zones.byId.village.x0), 20, 340, 90, 3, true));
console.log(write('world-gate.png', Math.max(0, KD.Zones.byId.gate.x0 - 40), 20, 160, 80, 5, true));
